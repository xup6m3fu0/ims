...
**CORS**：若只用瀏覽器開 `http://<Mac IP>:4000` 存取（同源），可設：

```env
CORS_ORIGIN=http://<你的 Mac 區網 IP>:4000
```
...
在 **同一台 Mac** 瀏覽器開：`http://127.0.0.1:4000`  
在 **同區網其他裝置** 開：`http://<Mac 的區網 IP>:4000`（例如 `http://192.168.1.50:4000`）

健康檢查：`http://<IP>:4000/healthz` 應回 `{"ok":true}`。
...
3. **防火牆**（若已開啟）：  
   - **系統設定 → 網路 → 防火牆 → 選項**，允許 `node` 接受連入連線，或新增規則開放 **TCP 4000**（或你設定的 `PORT`）。
...
| 想改用 80 埠 | `PORT=80` 需管理者權限執行 node，或前面加 **nginx** 反向代理到 4000 |
